...
type Point = { date: string; profit: number; operations: number };

// Прибуток по днях (полярні бари навколо нульової лінії).
...
export default function ProfitChart({ days = 14 }: { days?: number }) {
  ...
  const { bars, zeroY, maxIdx } = useMemo(() => {
    const maxAbs = Math.max(1, ...series.map((p) => Math.abs(p.profit)));
    const hasNeg = series.some((p) => p.profit < 0);
    const hasPos = series.some((p) => p.profit >= 0);
    ...
    const bars = series.map((p, i) => {
      const scale = p.profit >= 0 ? (zeroY - PAD_T) : (H - PAD_B - zeroY);
      const h = Math.abs(p.profit) / maxAbs * Math.max(scale, 1);
      return {
        ...p,
        x: i * step + (step - barW) / 2,
        y: p.profit >= 0 ? zeroY - h : zeroY,
        w: barW,
        h: Math.max(h, p.profit === 0 ? 0 : 1.5),
      };
    });
    let maxIdx = -1, best = 0;
    series.forEach((p, i) => { if (Math.abs(p.profit) > best) { best = Math.abs(p.profit); maxIdx = i; } });
    return { bars, zeroY, maxIdx };
  }, [series]);

  const total = series.reduce((s, p) => s + p.profit, 0);
    ...
    <div className="bg-white rounded-xl shadow p-5">
      <div className="flex items-baseline justify-between mb-2">
        ...
        <div className="text-right">
          ...
          <div className={`font-bold ${total >= 0 ? 'text-green-700' : 'text-red-600'}`}>
            ...
            {bars.map((b, i) => (
              <g key={b.date}>
                ...
                <rect
                  ...
                  fill={b.profit >= 0 ? POS : NEG}
                  opacity={hover === null || hover === i ? 1 : 0.35}
                  style={{ pointerEvents: 'none', transition: 'opacity .1s' }}
                />
                {/* вибіркова пряма мітка — лише пік (текст токеном, не кольором серії) */}
                {i === maxIdx && Math.abs(b.profit) > 0 && (
                  <text x={b.x + b.w / 2} y={b.profit >= 0 ? b.y - 4 : b.y + b.h + 11}
                    textAnchor="middle" fontSize="10" fill="#374151" fontWeight="600">
                    {b.profit >= 0 ? '+' : '−'}{Math.abs(b.profit) >= 1000 ? (Math.abs(b.profit) / 1000).toFixed(1) + 'k' : Math.abs(b.profit).toFixed(0)}
                  </text>
                )}
          ...
          {hovered && (
            ...
            >
              ...
              <div className="font-semibold">
                {hovered.profit >= 0 ? '+' : '−'}{Math.abs(hovered.profit).toFixed(2)} ₴
              </div>
              <div className="text-gray-400">{hovered.operations} опер.</div>
            </div>
          )}
        </div>
      )}
